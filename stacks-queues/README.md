# Stacks & Queues

LIFO/FIFO structures used for tracking history, parsing, and monotone problems.

## Key Concepts
- Monotonic stack (increasing / decreasing)
- Stack for bracket matching / expression evaluation
- Decode and undo patterns

## Problems

| # | Problem | Difficulty | Platform | File |
|---|---------|:---:|:---:|------|
| 1 | Min Stack | Medium | LeetCode 155 | [155_min_stack.py](./155_min_stack.py) |
| 2 | Decode String | Medium | LeetCode 394 | [394_decode_string.py](./394_decode_string.py) |
| 3 | Daily Temperatures | Medium | LeetCode 739 | [739_daily_temperatures.py](./739_daily_temperatures.py) |
| 4 | Valid Parentheses | Easy | LeetCode 20 | [20_Valid_Parentheses.py](./20_Valid_Parentheses.py) |
